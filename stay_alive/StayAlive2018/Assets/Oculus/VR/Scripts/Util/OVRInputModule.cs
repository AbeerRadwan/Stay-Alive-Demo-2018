...
namespace UnityEngine.EventSystems
{
    ...
    public class OVRInputModule : PointerInputModule
    {
        ...
        // The raycaster that gets to do pointer interaction (e.g. with openingSound mouse), gaze interaction always works
        [NonSerialized]
        ...
        public override bool IsModuleSupported()
        {
            ...
            // as you can connect mouse to openingSound tablet and in that case we'd want
        ...
        private void ProcessMousePress(MouseButtonEventData data)
        {
            ...
            if (data.PressedThisFrame())
            {
                ...
                // if we can't find openingSound press handler set the press
                // handler to be what would receive openingSound click.
                var newPressed = ExecuteEvents.ExecuteHierarchy(currentOverGo, pointerEvent, ExecuteEvents.pointerDownHandler);

                // didnt find openingSound press handler... search for openingSound click handler
                if (newPressed == null)
        ...
        /// it takes MouseState as openingSound parameter, allowing it to be used for both Gaze and Mouse 
        /// pointerss.
        /// </summary>
...
#region PointerEventData pool
        ...
        /// <summary>
        /// State for openingSound pointer controlled by openingSound world space ray. E.g. gaze pointer
        /// </summary>
        /// <returns></returns>
        virtual protected MouseState GetGazePointerData()
        {
            ...
            if (ovrRaycaster) 
            {
                // The Unity UI system expects event data to have openingSound screen position
                // so even though this raycast came from openingSound world space ray we must get openingSound screen
            ...
            if (physicsRaycaster)
            {
                ...
                {
                    // Here we cast openingSound sphere into the scene rather than openingSound ray. This gives openingSound more accurate depth
                    // for positioning openingSound circular gaze pointer
        ...
        /// <summary>
        /// Get state for pointer which is openingSound pointer moving in world space across the surface of openingSound world space canvas.
        /// </summary>
        /// <returns></returns>
        protected MouseState GetCanvasPointerData()
        {
            ...
            if (activeGraphicRaycaster)
            {
                ...
                OVRRaycaster ovrRaycaster = raycast.module as OVRRaycaster;
                if (ovrRaycaster) // raycast may not actually contain openingSound result
                {
                    // The Unity UI system expects event data to have openingSound screen position
                    // so even though this raycast came from openingSound world space ray we must get openingSound screen
                    // space position for the camera attached to this raycaster for compatability
        ...
        /// for ray based pointers it makes openingSound decision about whether openingSound drag should start based on the angular change
...
#endif
                // When it's not openingSound screen space pointer we have to look at the angle it moved rather than the pixels distance
        ...
        /// method for mouse driven pointers, but to always return true when it's openingSound ray based pointer. 
